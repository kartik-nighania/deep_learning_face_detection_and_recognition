...
def recognize():
   ...
   # Create an object of face aligner module
   image_size = (160, 160)
   ask = input("Enter desired face width and height in WidthxHeight format OR press ENTER for default 160x160 pixel: ").rstrip().lower()
   if ask != "":
   ...
   factor = 0.709 # scale factor
   ask = input("\nEnter the threshold DETECTION probability to consider detection by MTCNN OR press ENTER for default 0.80: ")
   if ask != "" and int(ask) < 1:
      threshold[2] = round(float(ask),2)

   classifier_threshold = 0.50
   ask = input("\nEnter the threshold RECOGNITION probability to consider face is recognised OR press ENTER for default 0.50: ")
   if ask != "":
   ...
   if input_type == "w":
        ...
        loop_type = True
        # Ask for webcam resolution
        ask = input("Enter your webcam SUPPORTED resolution for face detection. For eg. 640x480 OR press ENTER for default 640x480: ").rstrip().lower()
        if ask != "":
   ...
   # If image selected, trying to represent it as video with 1 frame
        ...
        loop_type = False
        total_frames = 0
        data_type = input("Write the image path file to open: ").rstrip().lstrip()
   ...
   # Video is selected
        ...
        loop_type = False
        data_type = input("Write the video path file to open: ").rstrip().lstrip()
       ...
       if bb.shape[0] > 0:
         ...
         for i in range(bb.shape[0]):
            ...
            if best_class_probabilities[i] > classifier_threshold:
               text = class_names[best_class_indices[i]] + " " + str( round(best_class_probabilities[i]*100, 2) ) + "%"
               cv2.putText(image, text, (int(bb[i][0]),int(bb[i][1])-7), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1,(0,0,255), 1, cv2.LINE_AA)
